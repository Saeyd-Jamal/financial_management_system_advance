                ...
                <div class="tab-content" id="pills-tabContent">
                    <div class="tab-pane fade show active pills-table" id="pills-table" role="tabpanel" aria-labelledby="pills-table-tab">
                        <div class="card-body table-container p-0">
                            <table id="savings-table" class="table table-striped table-bordered table-hover sticky" style="width:100%; height: calc(100vh - 168px);">
                                <thead>
                                    <tr>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                let table = $('#savings-table').DataTable({
                    ...
                    ajax: {
                        url: '{{ route("dashboard.savings.index") }}',
                        type: 'GET',
                        data: function (d) {
                            d.year = $('#year').val();
                            d._token = csrf_token;
                    ...
                    columns: [
                        ...
                        { data: 'total_savings', name: 'total_savings', orderable: false, render: function(data, type, row) {
                            let field = `<x-form.input name="total_savings_${row.id}" style="width: 100px; padding: 6px 5px;" type="number" value="${data}" step="0.01" min="0" />`;
                            return field;
                        }},
                        { data: 'edit', name: 'edit', orderable: false, searchable: false, render: function(data, type, row) {
                            @can('update','App\\Models\ReceivablesLoans')
                            let link = `<button class="btn btn-sm btn-icon text-primary edit_row"  style="padding: 1px;" data-id=":saving"><i class="fa-solid fa-edit"></i></button>`.replace(':saving', data);
                            return link ;
                            @else
                            return '';
                            @endcan
                        ...
                        { data: 'print', name: 'print', orderable: false, searchable: false, render: function (data, type, row) {
                            ...
                            return `
                                <form method="post" action="{{ route('dashboard.savings.print', ':savings' )}}" target="_blank">
                                    @csrf
                                ...
                                `.replace(':savings', data).replace(':year', $('#year').val());
                            @else
                            return '';
                            @endcan
                        }},
                ...
                function editReceivablesForm(id) {
                    $.ajax({
                        url: "{{ route('dashboard.savings.update', ':id') }}".replace(':id', id),
                        method: 'PUT',
                        headers: {
